...
#endif
{
   ...
   dtfixed_d()   = double(-1.0e-6) ; // Negative means use courant condition
   stoptime_d()  = double(1.0e-2); // *Real_t(edgeElems*tp/45.0) ;
   ...
   deltatimemultlb_d() = double(1.1) ;
   deltatimemultub_d() = double(1.2) ;
   dtcourant_d() = double(1.0e+20) ;
   dthydro_d()   = double(1.0e+20) ;
   dtmax_d()     = double(1.0e-2) ;
   time_d()      = double(0.) ;
   ...
   //deltatime_d() = (Real_t(.5)*cbrt(volo(0)))/sqrt(Real_t(2.0)*einit);
