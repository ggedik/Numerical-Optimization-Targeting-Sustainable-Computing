...
class Domain {
   ...
   void AllocateNodePersistent(Int_t numNode) // Node-centered
   {
      ...
      m_x_d.resize(numNode);  // coordinates
      m_y_d.resize(numNode);
      m_z_d.resize(numNode);

      m_xd.resize(numNode); // velocities
      m_yd.resize(numNode);
      m_zd.resize(numNode);

      m_xd_d.resize(numNode); // velocities
      m_yd_d.resize(numNode);
      m_zd_d.resize(numNode);
      ...
      m_xdd_d.resize(numNode); // accelerations
      m_ydd_d.resize(numNode);
      m_zdd_d.resize(numNode);
   ...
   double& x_d(Index_t idx)    { return m_x_d[idx] ; }
   double& y_d(Index_t idx)    { return m_y_d[idx] ; }
   double& z_d(Index_t idx)    { return m_z_d[idx] ; }
   ...
   double& xd_d(Index_t idx)   { return m_xd_d[idx] ; }
   double& yd_d(Index_t idx)   { return m_yd_d[idx] ; }
   double& zd_d(Index_t idx)   { return m_zd_d[idx] ; }

  
   // Nodal accelerations
  
   double& xdd_d(Index_t idx)  { return m_xdd_d[idx] ; }
   double& ydd_d(Index_t idx)  { return m_ydd_d[idx] ; }
   double& zdd_d(Index_t idx)  { return m_zdd_d[idx] ; }
   ...
   double& time_d()                 { return m_time_d ; }
   double& deltatime_d()            { return m_deltatime_d ; }
   double& deltatimemultlb_d()      { return m_deltatimemultlb_d ; }
   double& deltatimemultub_d()      { return m_deltatimemultub_d ; }
  //GG
  Real_t& stoptime()             { return m_stoptime ; }
  double& stoptime_d()             { return m_stoptime_d ; }
   ...
   double& dtcourant_d()            { return m_dtcourant_d ; }
   double& dthydro_d()              { return m_dthydro_d ; }
   double& dtmax_d()                { return m_dtmax_d ; }
   double& dtfixed_d()              { return m_dtfixed_d ; }
  ...
  private:
   ...
   std::vector<double> m_x_d ;  /* coordinates */
   std::vector<double> m_y_d ;
   std::vector<double> m_z_d ;
   ...
   std::vector<double> m_xd_d ; /* velocities */
   std::vector<double> m_yd_d ;
   std::vector<double> m_zd_d ;
   ...
   std::vector<double> m_xdd_d ; /* accelerations */
   std::vector<double> m_ydd_d ;
   std::vector<double> m_zdd_d ;
   ...
   double  m_dtcourant_d ;         // courant constraint 
   double  m_dthydro_d ;           // volume change constraint 
  ...
  double m_deltatime_d;
   ...
   double  m_deltatimemultlb_d ;
   Real_t  m_deltatimemultlb ;
   Real_t  m_deltatimemultub ;
  double  m_deltatimemultub_d ;
  
   Real_t  m_dtmax ;             // maximum allowable time increment 
   Real_t  m_stoptime ;          // end time for simulation 
   double  m_dtmax_d ;             // maximum allowable time increment 
   double  m_stoptime_d ;       
