...
for i in range (1,55):
    result =  subprocess.run(["/usr/bin/time","--format", "\"%U\n%S\"", "./reactor_simulator_mixed_prec_full_funcs_v3","1"," 1"," 1"," 5000000"], stdout=subprocess.PIPE, stderr= subprocess.PIPE)
    #capture_output=True, text=True)
    output = result.stdout
    output_err = result.stderr.decode('utf-8')
    #.strip('\"').split('\\n')
...
print(f"Saving :{np.abs(double_median - mixed_median)/np.abs((double_median))} \n")
