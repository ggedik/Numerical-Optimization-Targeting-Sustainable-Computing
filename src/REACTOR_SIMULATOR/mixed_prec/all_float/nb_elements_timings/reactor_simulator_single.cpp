 ...
 # include <ctime>
  ...
  int absorb_f ( int &seed );


  double cross ( double e );
  float cross_f ( float e );

 double dist2c ( double e, int &seed );
 //cross is called float 
 float dist2c_cf ( float e, int &seed );

double energy ( int &seed );
//the functions that call energy will call energy_f
//also the variable e will be in float
//and other functions that call energy are going to call it in single precision
// to avoid casting 
float energy_f (int &seed); 
...
float r8_max_f(float x, float y) ; 

double r8_uniform_01 ( int &seed );
float r8_uniform_01_f ( int &seed );

void scatter ( int &seed, double &e, double &mu, double &azm );
//void scatter_f ( int &seed, double &e, float &mu, float &azm );
//e is changed to float 
void scatter_f ( int &seed, float &e, float &mu, float &azm );

void source ( int &seed, double &e, double &mu, double &azm, double &x, 
  double &y, double &z );
//void source_f ( int &seed, double  &e, float &mu, float &azm, float &x, 
//float &y, float &z );
void source_f ( int &seed, float  &e, float &mu, float &azm, float &x, 
float &y, float &z );

void timestamp ( );
void update ( double mu,double azm,double d,double &x,double &y,double &z );
void update_f ( float mu,float azm,float d,float &x,float &y,float &z );
...
//
{
  ...
  if (argc < 4) 
  perror("./program  [print_tot_energy every 500 part] [print_result 0y/1n] [print only tot energy at end] [optional: number of particles] \n");


  printf("THIS IS MIXED PRECISION\n");
  int print_tot_energy_over_particules = atoi(argv[1]); 
  int print_result = atoi(argv[2]); 
  int print_only_tot_energy_at_end = atoi(argv[3]);
...
//
        else
        {
          ...
          scatter_f ( seed, e, mu, azm );
        }
      }

      etot = ea + er + et; //at each particle turn 
      if(print_tot_energy_over_particules == 0 && (part % 499 == 0))
      printf("%.16f\n", etot); 
      
    }

    //print only the total energy at the end: 
    if(print_only_tot_energy_at_end == 0){
      printf("%.16f\n", etot); 
...
//casting to be adressed
  u = r8_uniform_01_f ( seed );
...
//
{
  ...
  s = r8_abs_f ( sinf ( 100.0f * ( expf ( e ) - 1.0f ) ) 
   + sinf ( 18.81f * ( expf ( e ) - 1.0f ) ) );

  y = r8_max_f ( 0.02f, s );

    value = 10.0f * expf ( -0.1f / y );
...
float dist2c_cf ( float  e, int &seed )
{
 ...
 //casting to be addressed
  u = r8_uniform_01_f ( seed );

  value = - logf ( u ) / cross_f ( e );
...
float energy_f (int &seed)
  ...
  c = 1.0f / ( 2.0f * ( sqrtf ( emax ) - sqrtf ( emin ) ) );

  value = ( u / ( 2.0f * c ) + sqrtf ( emin ) );
  value = value * value;
...
//the casting here will be adressed when we transform r8_u_0
  u = r8_uniform_01_f ( seed );
  mu = - 1.0f + 2.0f * u;
...
//there is no casting here since for now we have not touched the energy fucntion
  e = energy_f ( seed );
