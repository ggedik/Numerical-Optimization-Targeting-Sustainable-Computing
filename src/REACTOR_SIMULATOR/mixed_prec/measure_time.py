...
for i in range (1,32):
    result =  subprocess.run(["/usr/bin/time","--format", "\"%U\n%S\"", "/home/g/ggedik/reaktor/mixed_prec/reactor_simulator_double","1"," 1"," 1"," 5000000"], stdout=subprocess.PIPE, stderr= subprocess.PIPE)
...
for i in range (1,32):
    result =  subprocess.run(["/usr/bin/time","--format", "\"%U\n%S\"", "/home/g/ggedik/reaktor/mixed_prec/reactor_simulator_mixed_prec","1"," 1"," 1"," 5000000"], stdout=subprocess.PIPE, stderr= subprocess.PIPE)
    #capture_output=True, text=True)
    output = result.stdout
    output_err = result.stderr.decode('utf-8')
    #.strip('\"').split('\\n')
